    ...
    TWzipcode.prototype = {
        ...
        reset: function (container, obj) {
            var self = this,
                ...
                county = '',
                list = {
                    'county': '<option data-hidden="true" value="縣市" selected disabled hidden>縣市</option>',
                    'district': '<option data-hidden="true" value="鄉鎮市區" selected disabled hidden>鄉鎮市區</option>'
                    // 'county': '',
                    // 'district': ''
                },
                tpl = [];
        ...
        init: function () {
            ...
            var self = this,
                ...
                countyId = role.county.data('id') || opts.countyId,
                districtName = role.district.data('name') || opts.districtName,
                districtId = role.district.data('id') || opts.districtId,
                zipcodeName = role.zipcode.data('name') || opts.zipcodeName,
                zipcodeId = role.zipcode.data('id') || opts.zipcodeId,
                zipcodePlaceholder = role.zipcode.data('placeholder') || opts.zipcodePlaceholder,
                readonly = role.zipcode.data('readonly') || opts.readonly;

            // Elements create
            $('<select/>')
                .attr({'name':countyName, 'id':countyId, 'style':'margin-right: 8px;'})
                .addClass("")
                .addClass(role.county.data('style') || ('undefined' !== typeof opts.css[0] ? opts.css[0] : ''))
                .appendTo(role.county.length ? role.county : container);

            $('<select/>')
                .attr({'name':districtName, 'id':districtId, 'style':'margin-right: 4px;'})
                .addClass("")
                .addClass(role.district.data('style') || ('undefined' !== typeof opts.css[1] ? opts.css[1] : ''))
                .appendTo(role.district.length ? role.district : container);

            $('<input/>')
                .attr({'type': 'hidden', 'name': zipcodeName, 'id':zipcodeId, 'maxlength':'5'})
            ...
            self.wrap = {
                ...
                'zipcode': container.find('input[type=hidden][name="' + zipcodeName + '"]:first')
            };
